Pro corr_grid, area_info,xlabels1,xlabels2,labels
;NAME: 
;corr_grid 
;PURPOSE: 
;create an image of correlation coefficients for area
;INPUT:
...
xyouts,.98,.05,'1',/normal
	...
	for i=0, elem_area-1 do begin 
		...
		for j=0,elem_area-1 do begin 
			corr[j] = correlate(*area_info[i],*area_info[j])
			corr_abs = abs(corr)
		endfor
