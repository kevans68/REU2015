Pro corr_grid_ap, area_info,lcc_info,xlabels1,xlabels2,labels
;NAME: 
;corr_grid_ap
;PURPOSE: 
;create an image of correlation coefficients for area and line core position 
;INPUT:
;area_info: array of area information 
;lcc_info: array of position information 
...
set_plot,'PS'
device, filename='/users/evans/iss_data/comp_ap.eps',/color,/encapsulated
device, xsize= 15, ysize=15
device, bits_per_pixel=8
;create grid 
contour,corr_block,xax,yax,title= 'Correlation Coefficients: Area-Position', ticklen=1.0,xtickinterval=1,ytickinterval=1,xtickname=replicate(' ',30) ,ytickname=replicate(' ',30),xminor=1,yminor=1,/nodata
...
xyouts,.98,.05,'1',/normal
	for i=0, elem_area-1 do begin
		...
		for j=0,elem_area-1 do begin 
			corr[j] = correlate(*area_info[i],*lcc_info[j])
			corr_abs = abs(corr)
		endfor
