Pro corr_grid_pi,position_info,intensity_info,xlabels1, xlabels2,labels
;NAME:
;corr_grid_pi 
;PURPOSE: 
;create an image of correlation coefficients for position and intensity 
;INPUT:
;postion_info: array of position information 
...
set_plot,'PS'
device, filename='/users/evans/iss_data/comp_pi.eps',/color,/encapsulated
device, xsize= 15, ysize=15
device, bits_per_pixel=8
;create grid 
contour,corr_block,xax,yax,title='Correlation Coefficients: Position-Intensity',xtickname=replicate(' ',30) ,ytickname=replicate(' ',30) ,ticklen=1.0,xtickinterval=1,ytickinterval=1,xminor=1,yminor=1,/nodata 

loadct,13,/silent
m=0
l=0

	for l=0, n_elements(lcc_info)-1 do begin
		    xyouts,.17+.075*l,.08,xlabels1[l],charsize=.6,/normal
	endfor

	for n=0,n_elements(lcc_info)-1 do begin
		       xyouts,.16+.075*n,.065,xlabels2[n],charsize=.6,/normal
	endfor

	for m=0, n_elements(lcc_info)-1 do begin
		      xyouts, .05,.13+.08*m, labels[m],charsize=.6,/normal 
	 endfor

xyouts,.01,.5, 'Positon',/normal
...
xyouts,.98,.05,'1',/normal
	...
	for i=0, elem_area-1 do begin 
		...
		for j=0,elem_area-1 do begin 
			corr[j] = correlate(*lcc_info[i],*intensity_info[j])
			corr_abs = abs(corr)
		endfor
